        ...
        link: function(scope, element, attrs, ngModel) {
          ...
          if (attrs.taDisabled) {
            ...
            if (_isReadonly) {
              ...
              element.removeClass('ta-disabled');
            ...
            // taDisabled only has an effect if the taBind element is an input or textarea or has contenteditable='true' on it.
            // Otherwise it is disabled by default
            scope.$watch(attrs.taDisabled, function(newVal, oldVal) {
              if (oldVal === newVal) return;
              if (newVal) {
                element.addClass('ta-disabled');
                // we changed to readOnly mode (taDisabled='true')
                if (element[0].tagName.toLowerCase() === 'textarea' || element[0].tagName.toLowerCase() === 'input') {
                  element.attr('disabled', 'disabled');
                }
                if (element.attr('contenteditable') !== undefined && element.attr('contenteditable')) {
