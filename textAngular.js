...
textAngular.directive("textAngular", ['$compile', '$window', '$document', '$rootScope', '$timeout', function ($compile, $window, $document, $rootScope, $timeout) {
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            scope.displayElements = {
                ...
                text: angular.element("<div contentEditable='true' ng-hide='showHtml' ta-bind='text' ta-shortcut ng-model='text' ></div>")
            };
...
}]).directive('taShortcut', [function () {
    // Port certain shotcuts from browser's default behavior to common editing shortcuts.
    // For FireFox.
    ...
    function Shortcuts(element) {
        ...
        element.on('keydown', function(e) {
            ...
            var toBeExecActions = findMatchActions();
            angular.forEach(toBeExecActions, function (action) {
                action.run(e);
            });
        });
        ...
        function findMatchActions() {
            var beExecActions = [];
            angular.forEach(actions, function (action) {
                var beExec = action.keyCodes.length > 0;
                angular.forEach(action.keyCodes, function (keyCode) {
                    beExec = beExec && pressed[keyCode];
                });

                if (beExec) beExecActions.push(action);
            });

            return beExecActions;
        }

        _this.register = function (combination, action) {
            var combKeys = combination.toLowerCase().split('+');

            var keyCodes = [];
            angular.forEach(combKeys, function (key) {
                key = key.trim();
                var keycode = findKeycode(key);
        ...
        _this.remove = function (combination) {
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            var shortcut = new Shortcuts(element);
            
            shortcut.register('ctrl+b', function() {
                scope.wrapSelection('bold', null);
            });
            shortcut.register('ctrl+i', function () {
                scope.wrapSelection('italic', null);
            });
            shortcut.register('ctrl+u', function () {
