...
.directive('taBind', ['taSanitize', '$timeout', '$window', '$document', 'taFixChrome', 'taBrowserTag', 'taSelection', 'taSelectableElements', 'taApplyCustomRenderers', 'taOptions', 'taExecCommand',
				function(taSanitize, $timeout, $window, $document, taFixChrome, taBrowserTag, taSelection, taSelectableElements, taApplyCustomRenderers, taOptions, taExecCommand){
	...
	return {
		...
		link: function(scope, element, attrs, ngModel){
			...
			var _taExecCommand = taExecCommand(attrs.taDefaultWrap);
				...
				if(!_isContentEditable){
					...
					element.on('keydown', function(event, eventData){
						...
						if(!_isReadonly){
							...
							}else if( (event.keyCode === 8 && !event.shiftKey) || (event.keyCode === 9 && event.shiftKey) )  {
								console.log('on backspace and shift-tab');
								onBackspace(event);
