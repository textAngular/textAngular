...
.directive('taBind', ['taSanitize', '$timeout', '$window', '$document', 'taFixChrome', 'taBrowserTag', 'taSelection', 'taSelectableElements', 'taApplyCustomRenderers', 'taOptions', '_taBlankTest', 'taExecCommand',
				function(taSanitize, $timeout, $window, $document, taFixChrome, taBrowserTag, taSelection, taSelectableElements, taApplyCustomRenderers, taOptions, _taBlankTest, taExecCommandProvider){
			...
			if(_isInputFriendly){
				if(!_isContentEditable){
					...
					function onBackspace(event) {
						...
						if (selection.start.element !== selection.end.element || selection.start.offset !== selection.end.offset){
							return;
						} else if (!selection.container.tagName) {
							return;
						} else if (selection.start.offset == 0 && !selection.container.tagName.match(BLOCKELEMENTS) 
                                && !selection.container.tagName.match(LISTELEMENTS)
                                && !selection.container.parentNode.className.match(/\bta-bind\b/i) ) {
                            taSelection.setSelectionToElementStart(selection.container.parentNode);
                            onBackspace(event);
							return;
                           ...
                           if (selection.container.tagName.match(/^li$/i) ) {
                            ...
                            //} else if (!selection.container.previousSibling && selection.container.tagName.match(INLINEELEMENTS)) {
                            ...
                            } else if (!selection.container.previousSibling || !selection.container.previousSibling.tagName ) {
                                /* if either there is no previous sibling, or the previous sibling is a textnode, 
                                 * defer to browser deault */
                                return;
                            } else if (!selection.container.previousSibling) {
                                return;
                            } else {
                                ...
                                console.log(selection.container.previousSibling);
                                var lastDescendant = findLastBlockOrListItem(selection.container.previousSibling);
                                ...
                                var orphanBrOfSelection = selection.container.querySelector(':scope >br:first-child:last-child');
                                appendContent(lastDescendant, selection.container);
                                if (orphanBr) lastDescendant.removeChild(orphanBr);
                                if (orphanBrOfSelection) lastDescendant.removeChild(orphanBrOfSelection);
                            }
