...
.factory('taExecCommand', ['taSelection', 'taBrowserTag', '$document', function(taSelection, taBrowserTag, $document){
	...
	var childElementsToList = function(elements, listElement, newListTag){
		...
		listElement.remove();
		selectLi($target.find('li')[0]);
	};    /** require an angular.element element */
	...
	var indentListItem = function (itemElement) {
		...
		if (!tagName || !tagName.match(/li/i)) {
			...
			taSelection.setSelectionToElementEnd(itemElement);
			return;
        } else if (!prev ){
    ...
    var outdentListItem = function (itemElement, defaultWrap) {
		...
		if (!tagName || !tagName.match(/li/gi) || !parent.tagName || !parent.tagName.match(/[ou]l/gi)) {
			...
			if (itemElement !== parent.lastChild) {
				newContainer = angular.element("<" + parent.tagName + ">");
			...
			taSelection.setSelectionToElementEnd(itemElement.childNodes[0]);
			return;
			...
			taSelection.setSelectionToElementEnd(wrapElem);
			return;
		} else {
			/* when parent of list container is not a list, move the current list item right after
			* it's previous sibling under the parent container. */
			//console.log('parent of parent is not a list element');
			if (itemElement !== parent.lastChild) {
				newContainer = angular.element("<" + parent.tagName + ">");
				while (itemElement.nextSibling) {
	...
	var outdentBlock = function (defaultWrap, $selected) {
		var sel, beginElement, endElement, items, current, done ;
		sel = taSelection.getSelection();
		...
		angular.forEach(items, function(item) {
			outdentListItem(item, defaultWrap);
		});
	};
		...
		return function(command, showUI, options){
			...
			if(selectedElement !== undefined){
				...
				if(command.toLowerCase() === 'insertorderedlist' || command.toLowerCase() === 'insertunorderedlist'){
					...
					return indentBlock(taDefaultWrap, $selected);
				}else if(command.toLowerCase() === 'outdent') {
					return outdentBlock(taDefaultWrap, $selected);
